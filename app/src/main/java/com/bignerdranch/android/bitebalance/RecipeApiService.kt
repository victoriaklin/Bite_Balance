...
class Converters {
    ...
    @TypeConverter
    fun fromIngredientsList(value: String): List<Ingredients> {
        val listType = object : TypeToken<List<Ingredients>>() {}.type
        return gson.fromJson(value, listType)
    }

    @TypeConverter
    fun toIngredientsList(list: List<Ingredients>): String {
        return gson.toJson(list)
    }

    // Similar converters for other complex types
    // Add methods for Map<String, Image>, Map<String, NutrientInfo>, List<Digest>
}
